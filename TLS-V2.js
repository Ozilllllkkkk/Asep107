  ...
  Promise.race([axiosPromise, timeoutPromise])
    ...
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mThanhPhucDev\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mNguyen Thanh Phuc\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
